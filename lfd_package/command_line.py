...
def main():
    ...
    thermal_cost_baseline = costs.calc_fuel_charges(class_dict=class_dict, office_only=True,
    ...
    ###########################
    electric_cost_baseline = costs.calc_electric_charges(class_dict=class_dict, office_only=True,
                                                         electricity_bought_hourly=class_dict['costs'].el)
    elf_electric_cost_new = costs.calc_electric_charges(class_dict=class_dict, office_only=True,
    ...
    tlf_thermal_cost_total = costs.calc_fuel_charges(class_dict=class_dict, office_only=True,
                                                     fuel_bought_hourly=tlf_fuel_use_list)

    ###########################
    # Electrical Cost Savings
    ###########################
    tlf_electric_cost_new = costs.calc_electric_charges(class_dict=class_dict, office_only=True,
    ...
    # Retrieve TES Size
    tes_size_pp = sizing.size_tes(chp_size=chp_size_pp, class_dict=class_dict)

    pp_tes_heat_flow_list, pp_tes_soc = \
        storage.calc_tes_heat_flow_and_soc(chp_gen_hourly_btuh=pp_chp_gen_btuh, tes_size=tes_size_pp,
    ...
    pp_boiler_dispatch_hourly = boiler.calc_aux_boiler_output_rate(chp_gen_hourly_btuh_dict=chp_gen_hourly_btuh_dict,
                                                                   tes_size=tes_size_pp, chp_size=chp_size_pp,
                                                                   class_dict=class_dict, load_following_type="PP",
                                                                   tes_heat_flow_btuh=pp_tes_heat_flow_list)
    # Convert from power to energy
    # TODO: Commented for SA analysis only
    ...
    # Electrical Cost Savings
    ###########################
    pp_electric_cost_new = costs.calc_electric_charges(class_dict=class_dict, office_only=True,
    ...
    ###########################
    # TODO: Commented for SA analysis only
    # pp_cost_data_dict = costs.calc_costs(thermal_cost_new=pp_thermal_cost_total, tes_size=tes_size_pp,
    #                                      electrical_cost_new=pp_electric_cost_new, pct_incentive=incentive_base_pct,
    #                                      thermal_cost_baseline=thermal_cost_baseline, class_dict=class_dict,
    ...
    # Electrical Cost Savings
    ###########################
    peak_electric_cost_new = costs.calc_electric_charges(class_dict=class_dict, office_only=True,
    ...
    test, prob = sensitivity_analysis(thermal_cost_new=elf_thermal_cost_sa,
                                      ...
                                      tes_size=elf_tes_size_sa, max_pct_incentive=0.5)
    print(prob)
    print("-----")
    print(test['S1'])
    ...
    # annual_el_sum = class_dict['demand'].annual_sum_el.to(ureg.megaWh)
    # annual_el_peak = class_dict['demand'].annual_peak_el
    # annual_hl_sum = class_dict['demand'].annual_sum_hl.to(ureg.megaWh)
    # annual_hl_peak = class_dict['demand'].annual_peak_hl.to(ureg.kW)
    #
    # # Sizing Calcs
    # tes_size_elf.ito(ureg.kWh)
    # tes_size_tlf.ito(ureg.kWh)
    # tes_size_pp.ito(ureg.kWh)
    ...
    #                                              class_dict=class_dict)
    # pp_total_co2 = emissions.calc_chp_emissions(electricity_bought_annual=sum(pp_electricity_bought_hourly),
    #                                             chp_fuel_use_annual=sum(pp_thermal_consumption_hourly_chp),
    #                                             ab_fuel_use_annual=sum(pp_thermal_consumption_hourly_ab),
    #                                             class_dict=class_dict)
    ...
    # pp_total_co2.ito(ureg.metric_ton)
    # peak_total_co2.ito(ureg.metric_ton)
    #
    # data_header = ["Variable Name", "Baseline", "", "ELF", "", "TLF", "", "PP PES", "", "PP Peak", ""]
    #
    # results_data = [
    #     ###########################
    #     # Baseline Demand
    ...
    #      round(chp_size_pp.magnitude, 2), chp_size_pp.units,
    ...
    #      round(tes_size_pp.magnitude, 2), tes_size_pp.units,
    ...
    #      round(pp_electric_energy_savings.magnitude, 2), pp_electric_energy_savings.units,
    #      round(peak_electric_energy_savings.magnitude, 2), peak_electric_energy_savings.units],
    ...
    #      round(pp_annual_electricity_bought.magnitude, 2), pp_annual_electricity_bought.units,
    #      round(peak_annual_electricity_bought.magnitude, 2), peak_annual_electricity_bought.units],
    #     ["Electrical Energy Sold", "N/A", "N/A", 0, '', 0, '',
    #      round(pp_annual_electricity_sold.magnitude, 2), pp_annual_electricity_sold.units,
    ...
    #      round(pp_chp_thermal_gen.magnitude, 2), pp_chp_thermal_gen.units,
    ...
    #      round(pp_tes_thermal_dispatch.magnitude, 2), pp_tes_thermal_dispatch.units,
    ...
    #      round(pp_boiler_dispatch.magnitude, 2), pp_boiler_dispatch.units,
    ...
    #      round(pp_thermal_energy_savings.magnitude, 2), pp_thermal_energy_savings.units,
    #      round(peak_thermal_energy_savings.magnitude, 2), peak_thermal_energy_savings.units],
    #     ["Electrical Energy Savings", "N/A", "N/A",
    #      round(elf_electric_energy_savings.magnitude, 2), elf_electric_energy_savings.units,
    #      round(tlf_electric_energy_savings.magnitude, 2), tlf_electric_energy_savings.units,
    #      round(pp_electric_energy_savings.magnitude, 2), tlf_electric_energy_savings.units,
    ...
    #      pp_total_energy_savings.magnitude, pp_total_energy_savings.units,
    ...
    #      round(pp_electric_cost_new.magnitude, 2), pp_electric_cost_new.units,
    ...
    #      round(pp_thermal_cost_total.magnitude, 2), pp_thermal_cost_total.units,
    ...
    #      round(pp_cost_data_dict["chp_installed_cost"].magnitude, 2), pp_cost_data_dict["chp_installed_cost"].units,
    ...
    #      round(pp_cost_data_dict["chp_om_cost"].magnitude, 2), pp_cost_data_dict["chp_om_cost"].units,
    ...
    #      round(pp_cost_data_dict["tes_installed_cost"].magnitude, 2), pp_cost_data_dict["tes_installed_cost"].units,
    ...
    #      round(pp_cost_data_dict["tes_om_cost"].magnitude, 2), pp_cost_data_dict["tes_om_cost"].units,
    ...
    #      round(pp_cost_data_dict["pp_rev"].magnitude, 2), pp_cost_data_dict["pp_rev"].units,
    ...
    #      round(pp_cost_data_dict["simple_payback"].magnitude, 2), pp_cost_data_dict["simple_payback"].units,
    ...
    #      round(pp_cost_data_dict["incentive_payback"].magnitude, 2), pp_cost_data_dict["incentive_payback"].units,
    ...
    #      round(pp_total_co2.magnitude), pp_total_co2.units,
    ...
    #                         tlf_electricity_bought_list=tlf_electricity_bought_hourly, demand_class=class_dict['demand'])
    ...
    #                        pp_electricity_sold_list=pp_electric_sold_list, demand_class=class_dict['demand'])
    # plots.pp_plot_thermal(pp_chp_gen_btuh=pp_chp_gen_btuh, pp_tes_heat_flow_list=pp_tes_heat_flow_list,
    #                       pp_boiler_dispatch_hourly=pp_boiler_dispatch_hourly, demand_class=class_dict['demand'])
    # plots.pp_plot_tes_soc(pp_tes_soc=pp_tes_soc, demand_class=class_dict['demand'])
    #
